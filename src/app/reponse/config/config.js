...
var ResponsiveState = /** @class */ (function () {
    function ResponsiveState(responsiveConfig) {
        ...
        var resize_observer = fromEvent_1.fromEvent(window, 'resize')
            .pipe(debounceTime_1.debounceTime(this._responsiveConfig.config.debounceTime), defaultIfEmpty_1.defaultIfEmpty(), startWith_1.startWith(this.getWidth('window')));
        var pixel_ratio_observer = fromEvent_1.fromEvent(window, 'onload')
            .pipe(defaultIfEmpty_1.defaultIfEmpty(), startWith_1.startWith(this.getDevicePixelRatio()));
        var device_observer = fromEvent_1.fromEvent(window, 'onload')
            .pipe(defaultIfEmpty_1.defaultIfEmpty(), startWith_1.startWith(this.getUserAgent()));
        var user_agent_observer = fromEvent_1.fromEvent(window, 'onload')
            .pipe(defaultIfEmpty_1.defaultIfEmpty(), startWith_1.startWith(this.userAgent_data()));
        var orientation_observer = fromEvent_1.fromEvent(window, 'orientationchange')
            .pipe(defaultIfEmpty_1.defaultIfEmpty(), startWith_1.startWith(this.getOrientation()));
        this.elementoObservar = resize_observer.pipe(map_1.map(this.sizeOperations));
        this.anchoObservar = resize_observer.pipe(map_1.map(this.sizeObserver));
        this.browserObserver = device_observer.pipe(map_1.map(this.browserName));
        this.pixelObserver = pixel_ratio_observer.pipe(map_1.map(this.pixel_ratio));
        this.deviceObserver = device_observer.pipe(map_1.map(this.device_detection));
        this.orientationObserver = orientation_observer.pipe(map_1.map(this.orientation_device));
        this.standardObserver = device_observer.pipe(map_1.map(this.standard_devices));
        this.ieVersionObserver = device_observer.pipe(map_1.map(this.ie_version_detect));
        this.userAgentObserver = user_agent_observer.pipe(map_1.map(this.userAgent_data));
    }
